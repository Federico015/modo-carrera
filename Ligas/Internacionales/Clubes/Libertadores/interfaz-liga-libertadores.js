    ...
    for (i = 0; i < finalStandings.length; i++){
        ...
        if (i === 0){
            ...
            if(finalStandings[i][0] !== 'Independiente'){
                ...
                copaLibertadores.textContent = 'Mundial de Clubes: ' + finalStandings[i-5][0] + ', ' + finalStandings[i-4][0] + ', '+ finalStandings[i-3][0] + ', ' + finalStandings[i-2][0] + ', ' + finalStandings[i-1][0] + ' y '+ finalStandings[i][0]+'.'
                info.appendChild(copaLibertadores)
            }
            else {
                const copaLibertadores = document.createElement('div')
                copaLibertadores.className = 'textInfo'
                copaLibertadores.textContent = 'Mundial de Clubes: ' + finalStandings[i-5][0] + ', ' + finalStandings[i-4][0] + ', '+ finalStandings[i-3][0] + ', ' + finalStandings[i-2][0] + ', ' + finalStandings[i-1][0] + ' e '+ finalStandings[i][0]+'.'
                info.appendChild(copaLibertadores)
            }
        }
        else if (i === 7){

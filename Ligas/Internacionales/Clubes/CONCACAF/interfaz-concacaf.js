...
function placeTeams(){
    ...
    for (i = 0; i < finalStandings.length; i++){
        ...
        if (i === 0){
            ...
            if(finalStandings[i][0] !== 'Inter Miami'){
                const mundialDeClubes = document.createElement('div')
                mundialDeClubes.className = 'textInfo'
                mundialDeClubes.textContent = 'Mundial de Clubes: ' + finalStandings[i-4][0] + ', '+ finalStandings[i-3][0] + ', ' + finalStandings[i-2][0] + ', ' + finalStandings[i-1][0] + ' y '+ finalStandings[i][0]+'.'
                info.appendChild(mundialDeClubes)
            }
            else {
                const mundialDeClubes = document.createElement('div')
                mundialDeClubes.className = 'textInfo'
                mundialDeClubes.textContent = 'Mundial de Clubes: ' + finalStandings[i-4][0] + ', '+ finalStandings[i-3][0] + ', ' + finalStandings[i-2][0] + ', ' + finalStandings[i-1][0] + ' e '+ finalStandings[i][0]+'.'
                info.appendChild(mundialDeClubes)
            }
        }
        else if (i === 5){
            ...
            if(finalStandings[i][0] !== 'Inter Miami'){
               ...
               // info.appendChild(copaSudamericana)
                ...
                const copaSudamericana = document.createElement('div')
                copaSudamericana.className = 'textInfo'
                copaSudamericana.textContent = 'Copa Sudamericana: ' + finalStandings[i-5][0] + ', ' + finalStandings[i-4][0] + ', '+ finalStandings[i-3][0] + ', ' + finalStandings[i-2][0] + ', ' + finalStandings[i-1][0] + ' e '+ finalStandings[i][0]+'.'
                //info.appendChild(copaSudamericana)
                }
        }
        else if (i === 12){
